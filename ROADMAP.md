...
## Phase 8: Universal Format Detection + Probe

Status: completed

Deliverables:

- `detect_format(path)` identifies safetensors, GGUF, PyTorch ZIP, legacy pickle, and diffusers directories from magic bytes
- `probe_model(path)` returns tensor count, names, shapes, dtypes, param count — header only, no tensor data loaded
- directory probing for diffusers-style layouts (model_index.json detection)
- safetensors probe reuses existing header parser for zero-cost integration

Tests: 10 unit tests covering format detection and probe for all supported formats.

## Phase 9: PyTorch Checkpoint Loading

Status: completed

Deliverables:

- safe pickle bytecode scanner — parses ~40 opcodes, extracts `_rebuild_tensor_v2`/v3 calls without executing arbitrary code
- ZIP tensor extraction with storage key resolution
- security allowlist — only torch/collections globals; `os`/`sys`/`subprocess` rejected
- state dict unwrapping for `{"state_dict": ...}`, `{"model": ...}`, etc.
- `load_pickle_index(path)` and `load_pickle_tensor(path, name)` PyO3 functions

Tests: 16 unit tests covering opcode parsing, security, dtype mapping, state dict unwrapping, end-to-end ZIP extraction.

## Phase 10: GGUF Reader + Dequantization Engine
...
- 31 quantization type enum with block_size/type_size computation
- GIL-released dequantization to BF16 for 19 quant types:
  - Simple: F16, F32, BF16, F64, I8, I16, I32, I64
  - Block quants: Q4_0, Q4_1, Q5_0, Q5_1, Q8_0, Q8_1
  - K-quants: Q2_K, Q3_K, Q4_K, Q5_K, Q6_K, Q8_K
- `QuantizedTensor` PyO3 class with lazy `.dequant()`, `.shape`, `.quant_type_name`, `.compression_ratio`
- IQ types preserved as QuantizedTensor but dequant returns clear error

Tests: 27 unit tests covering parser, all dequant kernels with hand-crafted block data, QuantizedTensor properties.

## Phase 11: Diffusers Directory Layout

Status: completed

Deliverables:

- `DiffusersLayout` parser — reads model_index.json, discovers components and weight files
- weight source discovery: single/sharded safetensors, single/sharded pytorch, or none
- `component_tensor_names` with `{component}/` prefixing
- `to_model_info` aggregates across all components
- `probe_diffusers(path)` PyO3 function

Tests: 9 unit tests covering layout discovery, sharded components, error cases.

## Phase 12: Unified load_model

Status: completed

Deliverables:

- single `load_model(path, strip_prefix=None)` entry point dispatching to all formats
- safetensors: mmap zero-copy path (reuses existing internals)
- GGUF: dense types eagerly dequanted to BF16, quantized types wrapped in QuantizedTensor
- PyTorch: ZIP extraction with dtype-aware tensor creation
- diffusers: component iteration with prefix aggregation
- `ModelData` Python class with dict-like access, `.tensors`, `.info`, `.format`
- `strip_prefix` parameter for removing `model.`/`module.` prefixes

Tests: 4 unit tests for dispatch helpers, format-specific dtype mapping.
